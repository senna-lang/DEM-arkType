import { Callable, appendUnique, flatMorph, includes, isArray, isEmptyObject, isKeyOf, printable, stringifyPath, throwError, throwInternalError } from "@ark/util";
...
export class BaseNode extends Callable {
    ...
    select(selector) {
        ...
        let nodes = NodeSelector.applyBoundary[normalized.boundary ?? "references"](this);
        if (normalized.kind)
            nodes = nodes.filter(n => n.kind === normalized.kind);
        if (normalized.where)
            nodes = nodes.filter(normalized.where);
        return NodeSelector.applyMethod[normalized.method ?? "filter"](nodes, this, selector);
    }
    ...
    configureDescendants(meta, selector = "references") {
        const normalized = NodeSelector.normalize(selector);
